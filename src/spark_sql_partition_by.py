...
aggregate using SQL-Partition-By

to execute:
spark-submit /home/assamese/work/python-projects/test1/spark_sql_partition_by.py
'''
...
class SparkApp:
    ...
    def run(sqlContext, csv_file_name, input_time, time_interval):
        ...
        partitionByStatus_sql = sqlContext.sql('''
          SELECT IP, Status, count(Status) 
            OVER 
            (
               PARTITION BY Status
               ORDER BY Status
            ...
            WHERE Status != '200'
        ''')
        print(partitionByStatus_sql.show())
